...
def rotateFallingPiece(data):
    ...
    newPce = list(  [False]*data.fallingPieceRows
                    for row in xrange(data.fallingPieceCols)    )   
    for row in xrange(data.fallingPieceRows):
        for col in xrange(data.fallingPieceCols):
            newPce[data.fallingPieceCols-1-col][row]=data.fallingPiece[row][col]
    data.fallingPiece = newPce
    data.fallingPieceRows = len(data.fallingPiece)
    data.fallingPieceCols = len(data.fallingPiece[0])
...
def tetrisStepFn(data):
    ...
    if (data.isGameOver == False):
        ...
        if (data.fallingPieceRow == 0) and (fallingPieceIsLegal(data) == False):
